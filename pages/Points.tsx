...
const PointsPage: React.FC = () => {
    ...
    // --- Core Analytics Engine (Strict Rule Enforcer) ---
    const analytics = useMemo(() => {
        ...
        results.forEach(result => {
            ...
            result.winners.forEach(winner => {
                ...
                if (rankPts === 0 && gradePts === 0) return;
    ...
    const itemOptions = useMemo(() => {
        ...
        return items.map(item => ({
            id: item.id,
            label: item.name,
            group: categories.find(c => c.id === item.categoryId)?.name
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-24">
                ...
                <div className="flex items-center gap-3 p-1.5 bg-zinc-100 dark:bg-white/5 rounded-2xl border border-zinc-200 dark:border-white/5 shadow-inner w-full lg:w-auto">
                    {[
                        ...
                        <button 
                            ...
                            className={`flex-1 lg:flex-none flex items-center justify-center gap-2 rounded-xl text-xs font-bold transition-all ${viewFilter === f.id ? 'bg-white dark:bg-zinc-700 text-amazio-primary dark:text-white shadow-sm ring-1 ring-black/5' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
                        >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                ...
                <div className="xl:col-span-8 space-y-6">
                                ...
                                return (
                                    <div key={team.teamId} className="group flex flex-col gap-1">
                                        <div 
                                            ...
                                            className={`relative overflow-hidden cursor-pointer p-4 rounded-3xl border transition-all duration-500 hover:scale-[1.01] ${isExpanded ? 'bg-indigo-50/50 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800 shadow-lg' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 shadow-sm'}`}
                                        >
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner ${idx === 0 ? 'bg-amber-400 text-amber-950' : idx === 1 ? 'bg-zinc-300 text-zinc-800' : idx === 2 ? 'bg-orange-300 text-orange-950' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>
                                                        {idx + 1}
                                                    </div>
                                        ...
                                        {isExpanded && (
                                            <div className="ml-4 sm:ml-12 mt-1 space-y-3 animate-in slide-in-from-top-4 duration-500">
                                                {(Object.values(team.categories) as CategoryBreakdown[]).map(cat => {
                                                    ...
                                                    const itemsInCat = (Object.values(cat.items) as ItemContribution[]).filter(item => 
                                                        item.itemName.toLowerCase().includes(globalSearchTerm.toLowerCase()) ||
                                                        item.contributors.some(c => c.participantName.toLowerCase().includes(globalSearchTerm.toLowerCase()))
                                                    );
                                                    ...
                                                    return (
                                                        <div key={cat.categoryId} className="bg-white/40 dark:bg-white/[0.01] rounded-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden">
                                                            <button 
                                                                ...
                                                                className="w-full flex items-center justify-between p-3 hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors"
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    <Layers size={14} className="text-zinc-400" />
                                                                    <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">{cat.categoryName}</span>
                                                                </div>
                                                                <div className="flex items-center gap-4">
                                                                    <div className="flex gap-1.5">
                                                                        <PointChip type="rank" value={cat.rankTotal} />
                                                                        <PointChip type="grade" value={cat.gradeTotal} />
                                                                    </div>
                                                                    <ChevronDown size={14} className={`text-zinc-400 transition-transform ${isCatExpanded ? 'rotate-180' : ''}`} />
                                                                </div>
                                                            </button>
                                                                ...
                                                                <div className="p-2 space-y-2 bg-zinc-50/50 dark:bg-black/20 animate-in fade-in duration-300">
                                                                    {itemsInCat.map(item => (
                                                                        <div key={item.itemId} className="p-3 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
                                                                            <div className="flex justify-between items-start mb-3 border-b border-zinc-50 dark:border-zinc-800 pb-2">
                                                                                <div>
                                                                                    <h4 className="text-xs font-bold text-zinc-800 dark:text-zinc-100">{item.itemName}</h4>
                                                                                    <div className="flex items-center gap-2 mt-0.5">
                                                                                        <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${item.performanceType === PerformanceType.ON_STAGE ? 'bg-purple-50 text-purple-600 border border-purple-100 dark:bg-purple-900/20 dark:border-purple-800' : 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:border-emerald-900/20 dark:border-emerald-800'}`}>
                                                                                            {item.performanceType}
                                                                                        </span>
                                                                                        <span className="text-[8px] text-zinc-400 uppercase font-black tracking-widest">{item.itemType}</span>
                                                                                    </div>
                                                                                </div>
                                                                                <div className="flex gap-2">
                                                                                    <PointChip type="rank" value={item.rankTotal} />
                                                                                    <PointChip type="grade" value={item.gradeTotal} />
                                                                                </div>
                                                                            </div>
                                                                            
                                                                            <div className="space-y-1.5">
                                                                                {item.contributors.map((p, pIdx) => (
                                                                                    <div key={`${p.participantId}-${pIdx}`} className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors group/row">
                                                                                        <div className="flex items-center gap-3 min-w-0">
                                                                                            <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-black ${p.rank === 1 ? 'bg-amber-400 text-amber-950' : p.rank === 2 ? 'bg-zinc-300 text-zinc-800' : p.rank === 3 ? 'bg-orange-300 text-orange-950' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'}`}>
                                                                                                {p.rank || '-'}
                                                                                            </div>
                                                                                            <div className="truncate">
                                                                                                <p className="text-xs font-medium text-zinc-700 dark:text-zinc-300">
                                                                                                    {p.participantName} {p.place && <span className="opacity-50 font-medium text-[0.8em]">, {p.place}</span>}
                                                                                                </p>
                                                                                                <div className="flex items-center gap-1.5">
                                                                                                    <p className="text-[9px] text-zinc-500 font-mono">#{p.chestNumber}</p>
                                                                                                </div>
                                                                                            </div>
                                                                                        </div>
                                                                                        <div className="flex items-center gap-4">
                                                                                            <div className="flex gap-1.5 opacity-60 group-hover/row:opacity-100 transition-opacity">
                                                                                                {p.rankPoints > 0 && <span className="flex items-center gap-1 text-[9px] font-bold text-amber-600 dark:text-amber-500"><Trophy size={10}/> +{p.rankPoints}</span>}
                                                                                                {p.gradePoints > 0 && <span className="flex items-center gap-1 text-[9px] font-bold text-emerald-600 dark:text-emerald-500"><GraduationCap size={10}/> {p.gradeName} (+{p.gradePoints})</span>}
                                                                                            </div>
                                                                                            <div className="text-xs font-black text-indigo-600 dark:text-indigo-400">+{p.total}</div>
                                                                                        </div>
                                                                                    </div>
                                                                                ))}
                                                                            </div>
                    ...
                    <Card title={`Quick ${insightsMode === 'ITEM' ? 'Item' : 'Participant'} Insights`} action={
                        <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl">
                            <button 
                                onClick={() => { setInsightsMode('ITEM'); setItemInsightsId(''); }}
                                className={`p-1.5 rounded-lg transition-all ${insightsMode === 'ITEM' ? 'bg-white dark:bg-zinc-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-zinc-400 hover:text-zinc-600'}`}
                                title="Item Mode"
                            >
                                <Layout size={16} />
                            </button>
                            <button 
                                onClick={() => { setInsightsMode('PARTICIPANT'); setParticipantInsightsId(''); }}
                                className={`p-1.5 rounded-lg transition-all ${insightsMode === 'PARTICIPANT' ? 'bg-white dark:bg-zinc-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-zinc-400 hover:text-zinc-600'}`}
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="relative">
                                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1.5 ml-1">
                                        Select {insightsMode === 'ITEM' ? 'Event' : 'Student'}
                                    </label>
                                    
                                    {insightsMode === 'ITEM' ? (
                                        <SearchableDropdown
                                            options={itemOptions}
                                            value={itemInsightsId}
                                            onChange={setItemInsightsId}
                                            placeholder="Find event item..."
                                            icon={Layout}
                                        />
                                    ) : (
                                        <SearchableDropdown
                                            options={participantOptions}
                                            value={participantInsightsId}
                                            onChange={setParticipantInsightsId}
                                            placeholder="Find student..."
                                            icon={User}
                                        />
                                    )}
                                </div>
                                <div className="p-3 rounded-2xl bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/50 flex items-start gap-3">
                                    ...
                                    <p className="text-[10px] text-indigo-800 dark:text-indigo-200 leading-relaxed">
                                        {insightsMode === 'ITEM' 
                                            ? "Instantly audit how points were distributed for this specific event."
                                            : "View detailed point breakdown for a specific student across all their registered items."
                                        }
                                    </p>
                                </div>
                            </div>
                            ...
                            {(insightsMode === 'ITEM' ? itemInsightsId : participantInsightsId) ? (
                                <div className="p-4 bg-zinc-50 dark:bg-black/20 rounded-2xl border border-zinc-100 dark:border-zinc-800 animate-in fade-in slide-in-from-top-2 duration-300">
                                    {insightsMode === 'ITEM' ? (
                                        (() => {
                                            ...
                                            return (
                                                <div className="space-y-3">
                                                    {itemRes.sort((a,b) => (a.rank || 99) - (b.rank || 99)).map((e, idx) => (
                                                        <div key={idx} className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                <div className="w-8 h-8 rounded-lg bg-white dark:bg-zinc-800 flex items-center justify-center text-xs font-bold shadow-sm">
                                                                    {e.rank || '-'}
                                                                </div>
                                                                <div>
                                                                    <div className="text-xs font-bold text-zinc-800 dark:text-zinc-100">{e.participant.name}</div>
                                                                    <div className="text-[9px] text-zinc-500 uppercase">{state.teams.find(t => t.id === e.participant.teamId)?.name}</div>
                                                                </div>
                                                            </div>
                                                            <div className="text-xs font-black text-indigo-600 dark:text-indigo-400">+{e.total} Pts</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            );
                                            ...
                                            return (
                                                <div className="space-y-3">
                                                    {partRes.map((e, idx) => (
                                                        <div key={idx} className="flex items-center justify-between">
                                                            <div>
                                                                <div className="text-xs font-bold text-zinc-800 dark:text-zinc-100">{e.item.name}</div>
                                                                <div className="text-[9px] text-zinc-500 uppercase">{e.gradeName} Grade • Rank {e.rank || '-'}</div>
                                                            </div>
                                                            <div className="text-xs font-black text-indigo-600 dark:text-indigo-400">+{e.total} Pts</div>
                                                        </div>
                                                    ))}
                                                    <div className="pt-2 border-t border-zinc-200 dark:border-zinc-700 flex justify-between items-center">
                                                        <div className="text-[10px] font-black uppercase text-zinc-400">Total Accumulation</div>
                                                        <div className="text-sm font-black text-indigo-600 dark:text-indigo-400">
                                                            {partRes.reduce((sum, e) => sum + e.total, 0)} Pts
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                                ...
                                <div className="py-10 text-center border-2 border-dashed border-zinc-100 dark:border-zinc-800 rounded-3xl opacity-40">
                                    <ListFilter size={32} className="mx-auto mb-3 text-zinc-400" />
                                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Select an entry to audit points</p>
                                </div>
                            )}
                        </div>
                    </Card>
                ...
                <div className="xl:col-span-4 space-y-6">
                    <Card title="Individual Toppers" action={
                        <div className="flex gap-2">
                             <select 
                                value={topperTypeFilter} 
                                onChange={(e) => topperTypeFilterSet(e.target.value as any)}
                                className="bg-transparent border-none text-[10px] font-black uppercase tracking-wider text-zinc-500 focus:ring-0 cursor-pointer outline-none"
                             >
                             ...
                             <select 
                                value={topperCategoryFilter} 
                                onChange={(e) => topperCategoryFilterSet(e.target.value)}
                                className="bg-transparent border-none text-[10px] font-black uppercase tracking-wider text-zinc-500 focus:ring-0 cursor-pointer outline-none"
                             >
                        ...
                        <div className="space-y-4">
                            {topperInsights.length > 0 ? (
                                topperInsights.map((topper, idx) => (
                                    <div key={topper.participantId} className={`relative p-4 rounded-3xl border transition-all ${idx === 0 ? 'bg-amber-50/50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800 shadow-lg' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800'}`}>
                                        <div className="flex items-center gap-4">
                                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-lg ${idx === 0 ? 'bg-amber-400 text-amber-950 shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>
                                                {idx + 1}
                                            </div>
                                            <div className="min-w-0 flex-grow">
                                                <h4 className="text-sm font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-tight truncate">
                                                    {topper.name}
                                                </h4>
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{topper.teamName}</span>
                                                    <span className="text-[9px] text-zinc-300">•</span>
                                                    <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-widest">{topper.categoryName}</span>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-xl font-black text-indigo-600 dark:text-indigo-400">{topper.points}</div>
                                                <div className="text-[8px] font-bold text-zinc-400 uppercase tracking-widest">Total</div>
                                            </div>
                                        </div>
                                    </div>
                                ))
                                ...
                                <div className="py-12 text-center opacity-30">
                                    ...
                                    <p className="text-xs font-black uppercase tracking-[0.2em]">No Data in Range</p>
                                </div>
                            )}
                        </div>
                    </Card>
                    
                    <div className="p-6 rounded-[2.5rem] bg-gradient-to-br from-indigo-600 to-indigo-800 text-white shadow-xl shadow-indigo-500/20 relative overflow-hidden group">
                        ...
                        <div className="flex items-center gap-3 mb-6 relative z-10">
                            <div className="h-5 w-1.5 bg-white rounded-full shadow-[0_0_12px_rgba(255,255,255,0.4)]"></div>
                            <h3 className="text-xl font-black font-serif uppercase tracking-tighter">Live Status</h3>
                        </div>
                        <div className="space-y-4 relative z-10">
                            <div className="flex justify-between items-center pb-3 border-b border-white/10">
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Completion</span>
                                <span className="text-sm font-black">{Math.round((analytics.stats.declaredCount / Math.max(state.items.length, 1)) * 100)}%</span>
                            </div>
                            <div className="w-full bg-white/10 rounded-full h-1.5">
                                <div className="bg-white h-full rounded-full" style={{ width: `${(analytics.stats.declaredCount / Math.max(state.items.length, 1)) * 100}%` }}></div>
                            </div>
                            <p className="text-[10px] leading-relaxed opacity-80 font-medium">
                                Data is synchronized across all judicial endpoints in real-time. Calculations follow strict point distribution heuristics.
                            </p>
                        </div>
                    </div>
                </div>
